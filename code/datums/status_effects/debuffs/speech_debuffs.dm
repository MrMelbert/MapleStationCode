/datum/status_effect/speech
	...
	var/tts_filter = ""
	/// If TRUE, we modify per char. If FALSE, we modify per word.
	var/per_char = TRUE
...
/datum/status_effect/speech/proc/handle_message(datum/source, list/message_args)
	...
	if(per_char)
		...
		for(var/i = 1, i <= length(phrase), i += length(original_char))
			...
			final_phrase += apply_speech(original_char, i)
 ...
 */
/datum/status_effect/speech/proc/apply_speech(original_char, index)
...
/datum/status_effect/speech/slurring/apply_speech(original_char, index)
