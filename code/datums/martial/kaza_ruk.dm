...
/datum/action/neck_chop/Trigger(trigger_flags)
	...
	if (source.streak == "neck_chop")
		owner.visible_message(span_danger("[owner] assumes a neutral stance."), span_boldnotice("Your next attack is cleared."))
		owner.balloon_alert(owner, "cleared")
		source.streak = ""
	else
		owner.visible_message(span_danger("[owner] assumes the Neck Chop stance!"), span_boldnotice("Your next attack will be a Neck Chop."))
		owner.balloon_alert(owner, "next attack: neck chop")
...
/datum/action/low_sweep/Trigger(trigger_flags)
	. = ..()
	if(!.)
		return
	var/datum/martial_art/source = target
	if (source.streak == "low_sweep")
		...
		owner.visible_message(span_danger("[owner] assumes the Low Sweep stance!"), span_boldnotice("Your next attack will be a Low Sweep."))
		owner.balloon_alert(owner, "next attack: low sweep")
...
/datum/action/lung_punch/Trigger(trigger_flags)
	...
	if (source.streak == "quick_choke")
		...
		owner.visible_message(span_danger("[owner] assumes the Lung Punch stance!"), span_boldnotice("Your next attack will be a Lung Punch."))
		owner.balloon_alert(owner, "next attack: lung punch")
