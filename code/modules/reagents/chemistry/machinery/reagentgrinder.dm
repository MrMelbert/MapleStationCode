...
/obj/machinery/reagentgrinder/proc/replace_beaker(mob/living/user, obj/item/reagent_containers/new_beaker)
	...
	if(!QDELETED(new_beaker))
		if(!user.transferItemToLoc(new_beaker, src, silent = FALSE)) // Non-module change
			return
		beaker = new_beaker
...
/obj/machinery/reagentgrinder/ui_interact(mob/user)
	...
	if(isAI(user)) // Non-module change
	...
	var/choice = show_radial_menu(
		...
		require_near = !issilicon(user), // Non-module change
	)
	if(!choice)
		return
	switch(choice)
		...
		if("examine")
			to_chat(user, boxed_message(jointext(examine(user), "<br>")))
