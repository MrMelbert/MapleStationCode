...
/datum/unit_test/shapeshift_health/Run()
	for(var/spell_type in subtypesof(/datum/action/cooldown/spell/shapeshift))
		...
		if (shift_spell.convert_damage)
			...
			TEST_ASSERT_EQUAL(round(dummy.get_total_damage(), 1), dummy.maxHealth * EXPECTED_HEALTH_RATIO, "Failed to transfer damage from [shift_spell.shapeshift_type] to original form using [shift_spell.name].")
			TRIGGER_RESET_COOLDOWN(shift_spell)
			...
			TEST_ASSERT_EQUAL(round(shifted_mob.get_total_damage(), 1), shifted_mob.maxHealth * EXPECTED_HEALTH_RATIO, "Failed to transfer damage from original form to [shift_spell.shapeshift_type] using [shift_spell.name].")
			TRIGGER_RESET_COOLDOWN(shift_spell)
