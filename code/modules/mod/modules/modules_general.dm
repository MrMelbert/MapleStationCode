...
/obj/item/mod/module/jetpack/on_activation()
	mod.wearer.add_movespeed_modifier(full_speed ? /datum/movespeed_modifier/jetpack/fullspeed : /datum/movespeed_modifier/jetpack)
...
/obj/item/mod/module/thermal_regulator/on_active_process(seconds_per_tick)
	...
	if(user.body_temperature > temperature_setting)
		user.adjust_body_temperature(min(-1 KELVIN, (temperature_setting - user.body_temperature) * 0.08) * seconds_per_tick, min_temp = temperature_setting)
	else if(user.body_temperature < temperature_setting)
		user.adjust_body_temperature(max(1 KELVIN, (user.body_temperature - temperature_setting) * 0.08) * seconds_per_tick, max_temp = temperature_setting)
