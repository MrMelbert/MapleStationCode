...
/mob/living/proc/update_transform(resize = RESIZE_DEFAULT_SIZE)
	...
	// Nothing has been changed, nothing has to be done.
	if(!changed)
		return FALSE
	...
	animate(src, transform = ntransform, time = is_opposite_angle ? 0 : UPDATE_TRANSFORM_ANIMATION_TIME, dir = final_dir, easing = (EASE_IN|EASE_OUT))
