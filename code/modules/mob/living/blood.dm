...
/mob/living/carbon/proc/bleed_warn(bleed_amt = 0, forced = FALSE)
	// NON-MODULE CHANGE for blood
	if(!blood_volume || !client || HAS_TRAIT(src, TRAIT_NOBLOOD))
		return
...
/mob/living/proc/transfer_blood_to(atom/movable/AM, amount, forced)
	...
	blood_volume -= amount
	AM.reagents.add_reagent(blood.reagent_type, amount, get_blood_data(blood.reagent_type), bodytemperature)
	return TRUE
...
/mob/living/carbon/human/get_blood_type()
	if(HAS_TRAIT(src, TRAIT_HUSK) || isnull(dna)|| !blood_volume)
		return null
	...
	if(HAS_TRAIT(src, TRAIT_NOBLOOD))
		return null
	return GLOB.blood_types[dna.blood_type]
