...
/obj/machinery/airalarm/proc/check_danger(turf/location, datum/gas_mixture/environment, exposed_temperature)
	...
	if(danger_level)
		...
		if(pressure <= WARNING_LOW_PRESSURE && temp <= BODYTEMP_COLD_DAMAGE_LIMIT - 10 CELCIUS)
			...
			return
		if(pressure <= WARNING_LOW_PRESSURE && temp >= BODYTEMP_HEAT_DAMAGE_LIMIT + 5 CELCIUS)
			warning_message = "Danger! Low pressure and high temperature detected."
			return
		if(pressure >= WARNING_HIGH_PRESSURE && temp >= BODYTEMP_HEAT_DAMAGE_LIMIT + 5 CELCIUS)
		...
		if(temp <= BODYTEMP_COLD_DAMAGE_LIMIT - 10 CELCIUS)
			...
			return
		if(temp >= BODYTEMP_HEAT_DAMAGE_LIMIT + 5 CELCIUS)
