...
/obj/machinery/barsign/proc/set_sign(datum/barsign/sign, do_rename_area = TRUE)
	if(!istype(sign))
		return

	var/area/bar_area = get_area(src)
	if(change_area_name && sign.rename_area && do_rename_area)
...
/obj/machinery/barsign/proc/pick_sign(mob/user)
	...
	for(var/obj/machinery/barsign/other_sign as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/barsign))
		if(get_area(other_sign) == get_area(src))
			other_sign.set_sign_by_name(picked_name, FALSE)
			signs_swapped += 1
