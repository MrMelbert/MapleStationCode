...
/obj/machinery/newscaster/proc/create_channel(mob/user, channel_locked, cross_sector)
	...
	if(hard_filter_result)
		tgui_alert(usr, "Your channel name contains: (\"[hard_filter_result[CHAT_FILTER_INDEX_WORD]]\"), which is not allowed on this server.")
		return TRUE
...
/obj/machinery/newscaster/proc/create_story(mob/user, mob/user, channel_id)
	...
	var/temp_message = tgui_input_text(user, "Write your Feed story", "Network Channel Handler", feed_channel_message, max_length = MAX_BROADCAST_LEN, max_length = MAX_BROADCAST_LEN, multiline = TRUE)
