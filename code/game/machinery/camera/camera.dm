		...
		for(var/mob/potential_viewer as anything in GLOB.player_list)
			if(isAI(potential_viewer))
				...
				if(user.name == "Unknown")
					to_chat(ai, "[span_name(user)] holds <a href='byond://?_src_=usr;show_tablet=1;'>\a [itemname]</a> up to one of your cameras ...") // Non-module change : 516 byond://
				else
					to_chat(ai, "<b><a href='byond://?src=[REF(ai)];track=[html_encode(user.name)]'>[user]</a></b> holds <a href='byond://?_src_=usr;last_shown_paper=1;'>\a [itemname]</a> up to one of your cameras ...") // Non-module change : 516 byond://
				continue
	...
	if(istype(attacking_item, /obj/item/paper))
		...
		for(var/mob/potential_viewer in GLOB.player_list)
			...
			if(isAI(potential_viewer))
				...
				if(user.name == "Unknown")
					to_chat(ai, "[span_name(user.name)] holds <a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a> up to one of your cameras ...") // Non-module change : 516 byond://
				else
					to_chat(ai, "<b><a href='byond://?src=[REF(ai)];track=[html_encode(user.name)]'>[user]</a></b> holds <a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a> up to one of your cameras ...") // Non-module change : 516 byond://
				continue
			...
			if (potential_viewer.client?.eye == src)
				...
				to_chat(potential_viewer, "[span_name(user)] holds <a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a> up to your camera...") // Non-module change : 516 byond://
		return
