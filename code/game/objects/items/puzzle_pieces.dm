...
/obj/machinery/door/puzzle/keycard/attackby(obj/item/attacking_item, mob/user, list/modifiers)
	. = ..()
...
/obj/machinery/puzzle/keycardpad/attackby(obj/item/attacking_item, mob/user, list/modifiers)
	. = ..()
...
/obj/machinery/puzzle/password/interact(mob/user, list/modifiers)
	...
	if(isnull(pass_input) || !user.can_perform_action(src, ALLOW_SILICON_REACH) || !user.in_range_to_interact_with(src))
		return
