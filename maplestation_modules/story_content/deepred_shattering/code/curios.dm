...
/obj/item/snowglobe/evtra/tre
	desc = "A small glass globe filled with a miniature winter scene. Inside is a miniature model of a man."
...
/obj/item/snowglobe/evtra/gst
	desc = "A small glass globe filled with a miniature winter scene. Inside is a miniature model of a mysterious man."
	decc = "A small glass globe filled with a miniature winter scene. Inside is a miniature model of a grey room."

/obj/item/snowglobe/evtra/ndl
	desc = "A small glass globe filled with a miniature winter scene. Inside is a miniature model of a deity of light."
	decc = "A small glass globe filled with a miniature winter scene. Inside is a miniature model of a garden."
...
/obj/item/snowglobe/secret/proc/rotate(input)
	var/text = ""
	for(var/i = 1, i <= length(input), i++)
		var/current = text2ascii(input, i)
	...
	if(counter == cycle_target)
		counter = 0
	else
		counter++
	return text

/obj/item/snowglobe/secret/proc/rotate_all_data()
	var/new_data = list()
	for(var/line in data)
		new_data += rotate(line)
	data = new_data
...
/obj/item/snowglobe/secret/examine(mob/user)
	...
	if(isliving(user))
		rotate_all_data()
	if(counter == cycle_target)
		desc = start + pick(data)
	else
		desc = start + "<span class='upside_down'>" + pick(data) + "</span>"
