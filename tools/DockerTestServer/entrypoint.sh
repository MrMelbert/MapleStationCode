...
# =================
# What this will do:
# 1) Copy the default config/ files from the read-only volume to /MapleStation/config/
# 2) Override files under /MapleStation/config/ with files present in /gamecfg/
# 3) Process environment variables passed to the container into /MapleStation/config/
...
sed -i -r 's/(#|^)ADDRESS .*/ADDRESS '"$DB_HOST"'/' /MapleStation/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_LOGIN .*/FEEDBACK_LOGIN '"$DB_USER"'/' /MapleStation/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_PASSWORD .*/FEEDBACK_PASSWORD '"$DB_PASS"'/' /MapleStation/config/dbconfig.txt
...
envvar_override "TG_GAME_" "/MapleStation/config/game_options.txt"
...
cd /MapleStation
DreamDaemon maplestation.dmb -port 1337 -trusted -close -verbose
