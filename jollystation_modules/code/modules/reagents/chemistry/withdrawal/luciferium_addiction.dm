...
/datum/addiction/luciferium/withdrawal_stage_1_process(mob/living/carbon/affected_carbon, delta_time)
	...
	if(current_addiction_cycle >= WITHDRAWAL_STAGE1_START_CYCLE && DT_PROB(33, delta_time))
		affected_carbon.mind.active_addictions[type] -= 2
...
/datum/addiction/luciferium/withdrawal_stage_2_process(mob/living/carbon/affected_carbon, delta_time)
	...
	var/current_addiction_cycle = LAZYACCESS(affected_carbon.mind.active_addictions, type)
	if(current_addiction_cycle >= WITHDRAWAL_STAGE2_START_CYCLE && DT_PROB(33, delta_time))
		affected_carbon.mind.active_addictions[type] -= 2
...
/datum/addiction/luciferium/withdrawal_stage_3_process(mob/living/carbon/affected_carbon, delta_time)
	...
	affected_carbon.adjustBruteLoss(clamp(0.00002 * (current_addiction_cycle ** 2), 0.5, 8) * delta_time, FALSE)
	affected_carbon.adjustCloneLoss(clamp(0.00002 * (current_addiction_cycle ** 2), 0.5, 8) * delta_time, FALSE)

	if(current_addiction_cycle >= WITHDRAWAL_STAGE3_START_CYCLE && DT_PROB(33, delta_time))
		affected_carbon.mind.active_addictions[type] -= 2
